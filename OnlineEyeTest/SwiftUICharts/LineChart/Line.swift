...
//        print(stepHeight)
        // modified
        let globalOffset = type == "Number" ? 0.1 : 0
        return curvedLines ? Path.quadCurvedPathWithPoints(points: points, step: CGPoint(x: stepWidth, y: stepHeight), globalOffset: globalOffset) : Path.linePathWithPoints(points: points, step: CGPoint(x: stepWidth, y: stepHeight))
    }
    var closedPath: Path {
        let points = self.data.onlyPoints()
...
//                    .animation(.easeIn(duration: 1.6))
            ...
            self.path
                ...
                .rotationEffect(.degrees(180), anchor: .center)
                .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
//                .animation(Animation.easeOut(duration: 1.2), value: self.showFull)
                .onAppear {
                    self.showFull = true
            }
            .onDisappear {
            ...
            if(self.showIndicator) {
                IndicatorPoint(indicatorColor: lineColor)
